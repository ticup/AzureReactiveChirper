...
namespace Orleans.Azure.Samples.ReactiveChirper
{
    public class WebSocketHandler
    {
        ...
        public void OnMessage(string jsonstring)
        {
            try
            {
                ...
                switch (type)
                {
                    ...
                    case "Unfollow":
                        Unfollow((string)json.Username, (string)json.ToUnfollow);
                        break;
                    ...
                    case "RemoveMessage":
                        RemoveMessage((string)json.Username, (string)json.MessageId);
                        break;
        ...
        public void Unfollow(string username, string toUnfollow)
        {
            Dispatch(async () =>
            {
                Logger.Verbose("{0} unfollowing {1}", username, toUnfollow);
                var grain = GrainClient.GrainFactory.GetGrain<IUserGrain>(username);
                await grain.Unfollow(toUnfollow);
            });
        }
        ...
        public void RemoveMessage(string username, string messageId)
        {
            Dispatch(async () =>
            {
                Logger.Verbose("Removing message of {0} : {1}", username, messageId);
                var grain = GrainClient.GrainFactory.GetGrain<IUserGrain>(username);
                var result = await grain.RemoveMessage(Guid.Parse(messageId));
            });
        }
